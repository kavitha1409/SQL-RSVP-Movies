...
SELECT
  (SELECT COUNT(*) FROM director_mapping) as Total_Rows_Director_Mapping,
  (SELECT COUNT(*) FROM genre) as Total_Rows_Genre,
  (SELECT COUNT(*) FROM movie) as Total_Rows_Movie,
  (SELECT COUNT(*) FROM names) as Total_Rows_Names,
  (SELECT COUNT(*) FROM ratings) as Total_Rows_Ratings,
  (SELECT COUNT(*) FROM role_mapping) as Total_Rows_Role_Mapping;
...
SELECT country, COUNT(*) AS count_movies, year 
FROM movie
...
FROM (
    ...
    ROW_NUMBER() OVER (ORDER BY COUNT(g.movie_id) DESC) AS row_num
    FROM genre AS g
...
SELECT count(movie_id) AS movies, ratings.median_rating
FROM ratings AS ratings
INNER JOIN movie AS movie ON ratings.movie_id=movie.id
...
SELECT
  ...
  IF(SUM(CASE WHEN m.languages LIKE '%German%' THEN r.total_votes ELSE 0 END) > SUM(CASE WHEN m.languages LIKE '%Italian%' THEN r.total_votes ELSE 0 END), 'Yes', 'No') AS German_Movie_Is_Popular_Than_Italian_Movie
FROM movie AS m
INNER JOIN ratings AS r ON m.id = r.movie_id;
...
WITH top_actors AS (
  SELECT name AS actor_name, COUNT(DISTINCT r.movie_id) AS movie_count
...
FROM movie AS m
JOIN genre AS g ON m.id = g.movie_id AND g.genre = 'thriller'
...
SELECT 
    ...
    ROUND(AVG(ROUND(AVG(duration), 2)) OVER (ORDER BY genre ROWS BETWEEN 10 PRECEDING AND CURRENT ROW), 2) AS moving_avg_duration
FROM movie AS m 
...
WITH movie_dates AS (
    SELECT d.name_id, n.name, d.movie_id, m.date_published,
           LEAD(m.date_published) OVER (PARTITION BY d.name_id ORDER BY m.date_published, d.movie_id) AS next_date
    FROM director_mapping AS d
    JOIN names AS n ON d.name_id = n.id
    JOIN movie AS m ON d.movie_id = m.id
),
date_diffs AS (
    SELECT name_id, AVG(DATEDIFF(next_date, date_published)) AS avg_inter_movie_days
    FROM movie_dates
    GROUP BY name_id
),
final_output AS (
    SELECT d.name_id AS director_id, n.name AS director_name,
           ...
           ROUND(AVG(r.avg_rating), 2) AS avg_rating, SUM(r.total_votes) AS total_votes, MIN(r.avg_rating) AS min_rating,
           MAX(r.avg_rating) AS max_rating, SUM(m.duration) AS total_duration,
           ROW_NUMBER() OVER (ORDER BY COUNT(d.movie_id) DESC) AS director_rank
    FROM director_mapping AS d
    JOIN names AS n ON d.name_id = n.id
    JOIN movie AS m ON d.movie_id = m.id
    JOIN ratings AS r ON m.id = r.movie_id
    JOIN date_diffs AS a ON a.name_id = d.name_id
